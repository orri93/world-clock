...
void Display::showNumber(uint16_t value) {
  ...
  for (uint8_t i = 0; i < DISPLAY_NUMBER_OF_DIGITS - 1; i++) {
    if (leadingSpace && digits[i] == 0) {
      segments[i] = instance.encodeCharacter(' ');
    } else {
      leadingSpace = false;
      segments[i] = instance.encodeCharacter(static_cast<char>('0' + digits[i]));
    }
  }
  segments[DISPLAY_NUMBER_OF_DIGITS - 1] = instance.encodeCharacter(
    static_cast<char>('0' + digits[DISPLAY_NUMBER_OF_DIGITS - 1]));
